...
%% 
% Depending on your starting point, you can begin with step 1 (raw reads) or 
% skip ahead to step 2 if you already have taxonomy-assigned read counts.
%% Bioinformatic processing of sequencing data
% Authors: Wiley Barton
% 
% Created: 2025.02.04
%% Requirements:
% To successfully follow this tutorial, you need to have the following installed 
% on your system:
%% 
...
% follow the instructions available here: <https://github.com/opencobra/cobratoolbox 
...
% The function runSeqC begins by setting the working directory to the SeqC repository, 
% assessing environmental variables and ensuring they are correctly formatted.
% 
...
% an optional estimation of the size of outputs which will be generated, is calculated 
% below.
...
%% 
% Then, the following command constructs Docker build arguments using the previously 
...
% they are provided. This allows for a flexible and efficient configuration within 
% the SeqC pipeline, enabling customized behavior without requiring changes to 
% the base setup.
...
% on user input, and constructs a command to run the database creation script 
% with the chosen database and a human contamination filter.
...
% flags for debugging and retaining intermediate files, depending on user-defined 
% settings. The full command is then formatted with the input data directory, 
% sample IDs, and other required parameters.
...
% SeqC runs within a *Docker container*. The following commands are required:
%% 
...
% 
% Once the Docker image is built, the following commands will:
%% 
...
%% 
% And finally, this command starts the full SeqC processing pipeline:
...
% 
% In this tutorial, we will cover how to create a community microbiome from 
% metagenomic reads data. We will explain how to use Microbial Abundances Retrieved 
% from Sequencing data—automated NCBI Taxonomy (MARS) [1] to map metagenomic reads 
% to AGORA2 database [2]. We will then explain how to process the output of MARS 
% using the Microbiome Modelling Toolbox [3] to generate community microbiome 
% models. 
%% Section 1: Setup
% Here, we show the steps required to set up your device to support the various 
% functions used to create and analyse human-microbiome models for all three tutorials. 
% First, we need to have a copy of COBRA toolbox. COBRA Toolbox download instructions 
% can be found at <https://github.com/opencobra/cobratoolbox https://github.com/opencobra/cobratoolbox>
% 
...
%% 
% To decrease simulation times of the models, an industrial solver is recommended. 
% The supported solvers include:
%% 
% * ibm_cplex
% * tomlab_cplex
% * gurobi
% * mosek
%% 
% To check compatibility between your MATLAB version and supported solvers, 
% visit: <https://opencobra.github.io/cobratoolbox/stable/installation.html#solver-installation. 
% https://opencobra.github.io/cobratoolbox/stable/installation.html#solver-installation.> 
% Many solvers are available through free academic licenses. For modelling purposes 
% here, we recommomend using gurobi if possible.
% 
% To set your solver in MATLAB, use the changeCobraSolver function. If a solver 
% other than gurobi is used, replace the 'gurobi' in the code line below with 
% the solver name you are using, as found in the bullet list above.
...
%% 
% MATLAB also requires two additional toolboxes to be installed, the Statistics 
% and Machine Learning Toolbox and the Parallel Computing Toolbox. To verify that 
% these toolboxes are installed and enabled, you can use the following code: matlab.addons.isAddonEnabled.
...
%% 
% If the parallelEnabled and statisticsEnabled are both 1 (true), then both 
% toolboxes are correctly installed in MATLAB and ready to be used. If either 
% one is 0, we recommended adding the toolbox from Home tab -> Add-Ons -> Get 
% Add-Ons. The parallel computing toolbox is essential, as the code for creating 
% the mWMBs will not function without it. The Statistics and Machine Learning 
% toolbox is not required to create the mWBMs, but it is necessary to run the 
% statistical analyses described in tutorial 4.
% 
% Before we can start creating mWBMs, we need to set up the paths to the general 
% results directory, the metadata file, and the taxonomy-assigned reads table. 
% We will use the general results directory to create a folder structure where 
% we can easily store our results in pre-defined locations. This is helpful, as 
% most functions rely on the outputs of previously executed steps. The metadata 
% and reads tables undergo sanity checks. The variables required are as follows:
%% 
% * *resultDir* - The path where all the results of this tutorial will be stored. 
% This ensures all your results will be in one place and are easily accesible. 
% Make sure the the folder is accesible to you.
% * *metadataPath* - The location of the metadata file. This is used in both 
% the generation of human-microbiome models and the analysis of results towards 
...
% the reads file, the first column must be called Taxon. If not, the first column 
...
% output directories used in Persephone. The other variables will be defined as 
...
paths.Mars.readsTablePath = '';
%% 
% Now we will define the paths where we want to store all of our results. We 
% do not have to create these directories manually, the code will do it for us.
...
% the COBRA solver, and checks if the required toolboxes are installed (lines 
% 1–7). Additionally, it generates the folder structure for the results. *IMPORTANT*: 
% the output structure used in all three tutorials is generated with initPersephone.m. 
% The metadata and the reads table are put through sanity checks. initPersephone.m 
% ensures that, in the metadata, the columns containing the sample IDs and sample 
% sex have the correct headers and readable data types. It always assumes the 
% first column contains sample IDs. It will create a new, updated file and will 
% also update the metadata path accordingly. If you use the test data files, you 
% will see that the column "sample ID" has been changed to "ID" and the column 
% "gender" has been altered to "Sex". Additionally, the data in the "Sex" column 
% has been changed from M/F to male/female. *IMPORTANT*: if your own metadata 
% does not have alternative headers accounted for in the function, it will raise 
% an error. This can be easily fixed by changing your column headers to "ID" or 
% "Sex", depending on which data type is causing the issue. Finally, we test if 
% the sample IDs in the microbiome data match those in the metadata.
...
%% 
% The following ouputs are returned from initPersephone.m
%% 
...
% installed. If false parts of Persephone are skipped.
...
%% 
% We also need to update the metadata path to reflect the updated metadata file.
...
% In this section, we will run MARS to convert the taxonomic reads table into 
% an AGORA2-mapped and normalised relative abundance table. MARS works as follows:
%% 
% * First, if the taxonomic assignments and reads tables are separate, they 
% will be merged into a single dataframe.
% * It removes clade extensions from all taxonomic level names (e.g., _Firmicutes__A_ 
% and _Firmicutes__B_ will be merged to _Firmicutes_) to allow for optimal AGORA2 
% mapping.
% * It translates certain species names based on a predefined list to ensure 
% the species names match those used in the AGORA2 database.
% * All reads associated with a taxonomic identification that lack information 
% up to the specified taxonomic level are removed.
% * It maps each taxonomic level (kingdom, phylum, species, etc.) to the AGORA2 
% database. If the taxonomic identification matches, it means there is a model 
% present.
...
%% 
% IMPORTANT: Before running this section, note that MARS can also be ran online 
% in your browser at:  <https://mars-pipeline.streamlit.app. https://mars-pipeline.streamlit.app.> 
% he website explains what inputs are required, and the variables you can use 
% are the same as explained here. It is important that you download the present_species.csv 
% file (file exentsion can differ) and save it in either the 'present' directory 
% in resultMARS named as 'present_species.csv', or provide the correct path to 
% MgPipe (discussed in the next section). This ensures that the MARS output file 
% can still be found by the subsequent functions.
% 
% If you want to run MARS offline, you need to make sure that all dependencies 
% are installed. Follow the code below to install all dependencies and obtain 
% your python path.
% 
...
marsRepoPath = '';
%% 
% First, we will test if Python is coupled with MATLAB. Then, we enter the cloned 
% MARS repository and add MARS as a readable module. In order to call the apporiate 
% function, we have to be inside the MARS directory.
...
% Required inputs:
%% 
% * *readsTable* - The path to the taxonomic reads file containing the number 
% of reads per taxonomic assignment. If the taxonomic assignments are included 
% in the reads file, the first column needs to be called Taxon. If not, the first 
% column of the readsFile and the first column of taxaTable needs to have to same 
% name. See the example files. (we set this already at the start).
% * *taxaTable* - The path to the taxonomic assignments for the taxomomic unit 
% name used by your taxonomic assignment software (e.g., OTU, ASV, OGU). This 
% has to be set to string(missing) if your readsTable already includes taxonomic 
% assignment. Otherwise, it must contain one column with a header matching the 
% first column in readsTable, and a second column with the header Taxon. 
% * *outputPathMars* - The path where the MARS results should be stored. We 
% created this directory in the section 1, and it is stored in paths.mars.
%% 
% Optional inputs:
%% 
% * *sample_read_counts_cutoff* - Numeric value for total read counts per sample 
% below which samples are excluded from analysis. Only applies when the readsTable 
% contains absolute read counts (not relative abundances). Defaults to 1, with 
% minimum of 1.
% * *cutoffMars* - The cutoff below which all relative abundances will be considered 
% 0. If smaller relative abundances are kept, numerical difficulties/infeasibilities 
% could occur later when solving the microbiome models. Also, relative abundances 
% below this value are unlikely to have a noticable impact on the flux results 
...
% Defaults to true.
% * *whichModelDatabase*: A string defining whether AGORA2, APOLLO, a combination 
% of both or a user-defined database should be used as model database for model 
% presence checks.  Allowed Input (case-insensitive): "AGORA2", "APOLLO", "full_db", 
...
%% 
% We now know which inputs we need to define. Let's start with the required 
% ones.
...
%% 
% Now let's set the optional inputs. You can adjust these according to your 
% own dataset; for the test dataset, we recommend using the settings provided 
% here.
...
userDatabase_path="";
%% 
% We have now defined all our variables for MARS. To pass the optional arguments, 
% we need to convert them into paired arguments readable by Python using the pyargs 
% function. We will store these converted inputs in the marsOptArg variable.
...
%% 
% The file renornmalized_mapped_forModelling.csv (file extension can change 
% based on user input), located in the renornmalized_mapped_forModelling directory, 
% is used as input to create community microbiome models. When using whole-genome 
% shotgun data, about 70–80% of the total reads can typically be mapped to the 
% AGORA2 database. For 16S sequencing data, this can drop to 30–40%, primarily 
% because many reads lack species-level information and are automatically discarded.
% 
% *NOTE:*
% 
% If the normalized_unMappedFile_Species file shows many species as absent, 
% it may be worthwhile to check for homosynonyms. You can either do this manually 
% or run your list of absent species through the MARS-ANT pipeline, which checks 
% for homosynonyms and presence in the AGORA2 resources: <https://mars-pipeline.streamlit.app 
% https://mars-pipeline.streamlit.app>.
% 
% If you use the online app, please store the files in the appropriate location 
% within your resultDir defined at the beginning of this part of the tutorial. 
% This allows for seamless integration with other functions and eliminates the 
% need to generate new path variables.
% 
% If you found homosynonyms manually, you will need to adjust the names of your 
...
% Modelling Toolbox 2.0 [3]. We will use pan-species models from the AGORA2 resource 
% [2]. _Pan-species_ means that all the strains of a species are combined into 
% one general model that includes all the metabolic capabilities of the individual 
% strains. This is done because metagenomic sequencing typically can only resolve 
% taxonomy up to the species level.
% 
% We will explain how the pipeline generates community metabolic models and 
% how to call the pipeline (MgPipe). The Microbiome Modelling Toolbox performs 
% the following steps:
%% 
...
% are adapted. This prepares the models to exchange metabolites with the internal 
...
% * All reactions present in each reconstruction, along with their associated 
% subsystems, are stored. These will be used to calculate reactionsPresence, reactionAbundance 
% and subsystemAbundance, can be used to describe the metabolic capabilities of 
% indiviual microbiome models.
% * All microbe reconstructions that are present in a sample are added together 
% in one large model. The reconstructions can exchange metabolites in the [u] 
% compartment created in the beginning of the function. The resulting microbiome 
% model can then exchange metabolites with the outside environment by transporting 
% metabolites from [u] to [e] and vice versa.
% * A microbiome biomass reaction is created in the form: a microbe1 + b micobre2 
% + c microbe3 + ... The letters a,b and c are the relative abundance for the 
% respective microbes as found in the relative abundance file we calculated via 
% MARS.
% * A standard diet is added to the microbiome models, and the model is tested 
% to confirm whether a total growth rate of 1 microbiome exchange per day (read 
% as 1 fecal exchange per day) can be achieved. 
% * Statistics on total amount of reactions, metabolites and microbes for each 
% microbiome are collected and visualised using violin plots.
%% 
% To run MgPipe, we first need to understand the required inputs. NOTE: In this 
% tutorial we only use a small subset of inputs for MgPipe, if you want a more 
% detailed explanation of the various inputs for the pipeline please look at: 
...
% can be downloaded from <https://doi.org/10.7910/DVN/JAXTWY https://doi.org/10.7910/DVN/JAXTWY>
...
% it yourself
% * computeProfiles - A boolean (true or false) that tells the pipeline whether 
% to calculate the exctretion and uptake profiles of the microbiome models. We 
% will not do this during this tutorial. However, if you are more interested in 
% the microbiome output, you can put this variable to true. This is a time-consuming 
% step and is sped-up by using gurobi as solver. For more information on the extra 
% output generated if this is set to true look at this tutorial in the COBRA toolbox: 
% <https://github.com/opencobra/COBRA.tutorials/tree/0d5becf7171844e53d3d43437035917a2bd9cc61/analysis/microbiomeModelingToolbox 
% https://github.com/opencobra/COBRA.tutorials/tree/0d5becf7171844e53d3d43437035917a2bd9cc61/analysis/microbiomeModelingToolbox>
%% 
% Optional inputs:
...
% More workers generally mean faster computation times. If you are unsure about 
% the appropriate value, follow the instructions in the code below to help you 
% decide.
% * mgPipeRes - The path where MgPipe results and the microbiome community models 
% will be stored. The directory is stored in the paths variable created in section 
% 1
%% 
...
%% 
% It can be difficult to define the number of workers you want to use if you 
% are unfamiliar with the concept. Here are some tips to help you: Running feature('numcores') 
% will tell you the number of available cores on your device. A good automatic 
% way to choose the number of workers is to use about 80% of your available cores, 
% leaving some cores free for non-MATLAB tasks. If you are working on a cluster, 
% or your device isn't needed for anything else, you can use all available cores. 
% This is recommended when the number of cores is greater than 4. If fewer than 
% 4, set the number of workers to 2 or 3. Note that you cannot set fewer than 
% 2 workers.
% 
% Below is the code that automatically sets the number of workers for you:
...
%% 
% Warnings you might encounter include: 
%% 
...
% 
% If the following errors occurs, follow the advice below:
% 
...
% 
% If no errors appear, the microbiome models should now be created. You can 
% find them in your output folder under mgpipeResults. In this folder, you will 
% also find several CSV files:
%% 
% * GrowthRates - It contains the maximum growth rate of each microbiome under 
% a rich diet and a pre-defined diet. 
% * ModelStatistics - For each microbiome model, the file reports the total 
% amount of reactions, metabolites and microbes. MicrobiomeModel_sizes.png is 
% a visual representation of this information.
% * ModelStatsSummary - Provides the mean, median, min and max of the amount 
% reactions metabolites and microbes considering all microbiome models. 
% * ReactionAbundance - Shows the relative abundance of a reaction for each 
% metabolic reaction in each microbiome model. This is based on the relative abundance 
% of the microbe and whether the microbe has that specific reaction.
% * ReactionPresence - Indicates whether  a reaction is present in at least 
% 1 microbe within a microbiome model.
...
% individual microbe models used to create the microbiome models are stored in 
% the modelStorage folder. infeasModels.mat contains information on models that 
% could not produce biomass. mapInfo.mat is used to hot-start the pipeline in 
% case there were errors or MATLAB was unexpectedly stopped or crashed during 
